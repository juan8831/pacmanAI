...
class CornersProblem(search.SearchProblem):
    ...
    def isGoalState(self, state):
        cornersVisted = len(state[1])
        return cornersVisted == 4
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hitsWall:
                ...
                if nextLocation in self.corners:
                    if nextLocation not in cornersVisited:
                        cornersVisited = cornersVisited + (nextLocation,) # corner that has not been visited yet
                successors.append(((nextLocation, cornersVisited), action, self.costFn(nextLocation)))
        ...
        for pointB in points:
            ...
            if pointA != pointB:
                ...
                if (pointA,pointB) not in edgeList:
                    edges.push((pointA, pointB, euclideanDistance(pointA, pointB)), euclideanDistance(pointA, pointB))
...
class FoodSearchProblem:
    ...
    def __init__(self, startingGameState):
        ...
        print "start", self.start[1].count()
        self.walls = startingGameState.getWalls()
